...
export default function PurchaseOrdersPage() {
  ...
  function populateForm(po) {
    setPoType(po.po_type || 'General Expense')
    setDept(po.department || 'Production')
    setSupplier(po.supplier || '')
    setDescription(po.description || '')
    setAmount(po.amount || '')
    setInvoiceNo(po.invoice_no || '')
    setFabricType(po.fabric_type || '')
    setFabricYards(po.fabric_yards || '')
    setFabricRate(po.fabric_rate || '')
    // Extract bill link from notes
    const bl = po.notes?.match(/Bill:\s*(.+)/)?.[1]?.trim() || ''
    setBillLink(bl)
    setPoNotes(po.notes?.replace(/Bill:\s*.+/g, '').trim() || '')
    setQuantity('')
    setReason('')
    setLinkedSku('')
  }
  ...
  function resetForm() {
    ...
    setQuantity(''); setReason(''); setLinkedSku(''); setBillLink(''); setPoNotes('')
    setEditingPO(null)
  }
  ...
  async function savePO(e) {
    ...
    try {
      ...
      if (poNotes) notesParts.push(poNotes)

      const payload = {
        po_type: poType, department: dept,
      ...
      if (editingPO) {
        ...
        payload.submitted_by_name = user?.name
  ...
  async function deletePO(po) {
    ...
    setConfirmDel(null); setViewPO(null); load()
  }
  ...
  function printPO(po) {
    ...
    ${billUrl?`<div class="notes"><div class="lbl">Bill Attachment</div>${billUrl}</div>`:''}
    ${po.notes&&!po.notes.startsWith('Bill:')?`<div class="notes"><div class="lbl">Notes</div>${po.notes.replace(/Bill:.+\n?/,'').replace(/\n/g,'<br>')}</div>`:''}
          ...
          <div className="bg-white rounded-2xl border border-sand-200 p-6 space-y-4">
            ...
            {cleanNotes && (
              <div className="bg-sand-50 rounded-xl p-4">
                ...
                <div className="text-sm text-ink-600 whitespace-pre-wrap">{cleanNotes}</div>
              </div>
            )}
          ...
          <div className="bg-white rounded-2xl border border-sand-200 p-6">
            ...
            <form onSubmit={savePO} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="text-xs font-bold text-ink-400 uppercase tracking-wider">📎 Bill Link / URL</label>
                  <input type="text" value={billLink} onChange={e => setBillLink(e.target.value)} placeholder="Paste Google Drive, photo, or file link"
  ...
  return (
    <AppShell>
      <div className="flex items-center justify-between mb-6">
        ...
        {canCreatePO && (
          <button onClick={() => { resetForm(); setTab('form') }} className="px-4 py-2.5 text-xs font-bold text-white bg-ak-900 rounded-xl hover:bg-ak-800">+ New PO</button>
        )}
      </div>
        ...
        <div className="bg-white rounded-2xl border border-sand-200 overflow-hidden">
          {pos.length === 0 ? <p className="text-sm text-ink-300 py-12 text-center">No POs yet</p> : (
            <table className="ak-table">
              <thead><tr><th>Ref</th><th>Type</th><th>Supplier</th><th>Dept</th><th>Description</th><th className="text-right">Amount</th><th>Status</th><th>📎</th><th className="text-right">Actions</th></tr></thead>
              <tbody>
                {pos.map(po => {
